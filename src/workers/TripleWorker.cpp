...
TripleWorker::TripleWorker(Handle *handle)
    : Worker(handle)
{
    m_state       = new State();
...
bool TripleWorker::resume(const Job &job)
{
...
void TripleWorker::consumeJob()
{
    ...
    memcpy(m_state->blob + m_state->job.size() + m_state->job.size(), m_state->job.blob(), m_state->job.size());
    ...
    if (m_state->job.isNicehash()) {
        ...
        m_state->nonce1 = 0xffffffffU / (m_threads * 3) * m_id;
        m_state->nonce2 = 0xffffffffU / (m_threads * 3) * (m_id + m_threads);
        m_state->nonce3 = 0xffffffffU / (m_threads * 3) * (m_id + m_threads + m_threads);
    }
}


void TripleWorker::save(const Job &job)
{
